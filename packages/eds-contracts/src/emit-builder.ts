/** Plan → runnable Figma script. The builder is a generic interpreter: it reads the
 *  embedded PLAN and constructs whatever it describes — variables ensured, one
 *  component per variant, parts by capability kind, bindings by name (structural
 *  bindings picked per the variant's size-axis value), axes combined, props wired.
 *  Deterministic: same plan, same component. No component-specific code.
 *
 *  Transport-agnostic: run via the Figma MCP `use_figma` or figma-console-mcp
 *  `figma_execute`. Returns a manifest for verification against the plan. */
import type { FigmaPlan } from './emit-figma.ts'
...
export function emitBuilder(plan: FigmaPlan): string {
  return (
    `// Generated builder for ${plan.component} — do not edit; edit the contract.\n` +
